...
<!--        {{ item.permissions }}-->
<!--    </pre>-->
    <div v-if="hasPermission(item.permissions)">
        <TransitionRoot class="space-y-2" appear :show="isShowing" enter="transition-opacity duration-75"
                        enter-from="opacity-0"
                        enter-to="opacity-100" leave="transition-opacity duration-300" leave-from="opacity-100"
                        leave-to="opacity-0">
            <Link
                v-if="!item.children.length && item.visible === true"
                :href="route(item.href)"
                @click="toggleSidebarVisibility" :class="[
            ...
            <Disclosure v-else-if="item.visible === true" v-slot="{ open }" :default-open="hasActiveChild">
                ...
                <span :class="['flex flex-1 gap-2',
                    ...
                    <svg
                        :class="['w-5 h-5 shrink-0', open ? '-rotate-0 transition duration-150 ease-linear' : '-rotate-90 transition duration-150 ease-linear']"
